...
Node* GraphKit::load_String_length(Node* ctrl, Node* str) {
  ...
  return _gvn.transform(new (C) RShiftINode(len, coder));
}
...
Node* GraphKit::capture_memory(const TypePtr* src_type, const TypePtr* dst_type) {
  ...
  MergeMemNode* merge = MergeMemNode::make(C, map()->memory());
...
Node* GraphKit::compress_string(Node* src, const TypeAryPtr* src_type, Node* dst, Node* count) {
  ...
  StrCompressedCopyNode* str = new (C) StrCompressedCopyNode(control(), mem, src, dst, count);
  Node* res_mem = _gvn.transform(new (C) SCMemProjNode(str));
...
void GraphKit::inflate_string_slow(Node* src, Node* dst, Node* start, Node* count) {
  ...
  Node* i_byte = new (C) PhiNode(head, TypeInt::INT);
  i_byte->init_req(1, intcon(0));
  gvn().set_type(i_byte, TypeInt::INT);
  record_for_igvn(i_byte);

  Node* i_char = new (C) PhiNode(head, TypeInt::INT);
