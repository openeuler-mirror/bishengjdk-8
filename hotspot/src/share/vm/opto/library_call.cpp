...
void LibraryCallKit::generate_string_range_check(Node* array, Node* offset, Node* count, bool char_count) {
  ...
  if (char_count) {
    ...
    count = _gvn.transform(new (C) LShiftINode(count, intcon(1)));
  }
...
Node* LibraryCallKit::make_string_method_node(int opcode, Node* str1_start, Node* cnt1, Node* str2_start, Node* cnt2, StrIntrinsicNode::ArgEnc ae) {
  ...
  switch (opcode) {
  case Op_StrIndexOf:
    result = new (C) StrIndexOfNode(control(), memory(TypeAryPtr::BYTES),
                                str1_start, cnt1, str2_start, cnt2, ae);
    break;
  case Op_StrComp:
    result = new (C) StrCompNode(control(), memory(TypeAryPtr::BYTES),
                             str1_start, cnt1, str2_start, cnt2, ae);
    break;
  case Op_StrEquals:
    ...
    result = new (C) StrEqualsNode(control(), memory(TypeAryPtr::BYTES),
...
bool LibraryCallKit::inline_string_equals(StrIntrinsicNode::ArgEnc ae) {
  ...
  if (!stopped()) {
    ...
    Node* cmp = _gvn.transform(new (C) CmpINode(arg1_cnt, arg2_cnt));
...
bool LibraryCallKit::inline_array_equals(StrIntrinsicNode::ArgEnc ae) {
  ...
  set_result(_gvn.transform(new (C) AryEqNode(control(), memory(mtype), arg1, arg2, ae)));
  return true;
}
...
bool LibraryCallKit::inline_hasNegatives() {
  ...
  Node* result = new (C) HasNegativesNode(control(), memory(TypeAryPtr::BYTES), ba_start, len);
...
bool LibraryCallKit::inline_string_indexOf(StrIntrinsicNode::ArgEnc ae) {
  ...
  if (ae == StrIntrinsicNode::UU || ae == StrIntrinsicNode::UL) {
    ...
    src_count = _gvn.transform(new (C) RShiftINode(src_count, intcon(1)));
  }
  if (ae == StrIntrinsicNode::UU) {
    // Divide substring size by 2 if String is UTF16 encoded
    tgt_count = _gvn.transform(new (C) RShiftINode(tgt_count, intcon(1)));
  }
...
bool LibraryCallKit::inline_string_indexOfI(StrIntrinsicNode::ArgEnc ae) {
  ...
  Node* src_offset = (ae == StrIntrinsicNode::LL) ? from_index : _gvn.transform(new (C) LShiftINode(from_index, intcon(1)));
  src_count = _gvn.transform(new (C) SubINode(src_count, from_index));
  ...
  Node* phi = new (C) PhiNode(region, TypeInt::INT);
  ...
  if (result != NULL) {
    ...
    Node* cmp = _gvn.transform(new (C) CmpINode(result, intcon(0)));
    ...
    if (!stopped()) {
      result = _gvn.transform(new (C) AddINode(result, from_index));
...
Node* LibraryCallKit::make_indexOf_node(Node* src_start, Node* src_count, Node* tgt_start, Node* tgt_count,
  ...
  Node* cmp = _gvn.transform(new (C) CmpINode(tgt_count, src_count));
  ...
  if (!stopped()) {
    ...
    cmp = _gvn.transform(new (C) CmpINode(tgt_count, intcon(0)));
...
bool LibraryCallKit::inline_string_indexOfChar() {
  ...
  Node* src_offset = _gvn.transform(new (C) LShiftINode(from_index, intcon(1)));
  Node* src_start = array_element_address(src, src_offset, T_BYTE);
  Node* src_count = _gvn.transform(new (C) SubINode(max, from_index));
  ...
  Node* result = new (C) StrIndexOfCharNode(control(), memory(TypeAryPtr::BYTES), src_start, src_count, tgt, StrIntrinsicNode::none);
  C->set_has_split_ifs(true); // Has chance for split-if optimization
...
bool LibraryCallKit::inline_string_copy(bool compress) {
  ...
  if (convert_src) {
    src_offset = _gvn.transform(new (C) LShiftINode(src_offset, intcon(1)));
  } else if (convert_dst) {
    dst_offset = _gvn.transform(new (C) LShiftINode(dst_offset, intcon(1)));
  }
...
bool LibraryCallKit::inline_string_toBytesU() {
  ...
  { PreserveReexecuteState preexecs(this);
    ...
    Node* size = _gvn.transform(new (C) LShiftINode(length, intcon(1)));
...
bool LibraryCallKit::inline_string_getCharsU() {
  ...
  Node* length = _gvn.transform(new (C) SubINode(src_end, src_begin));
  src_begin = _gvn.transform(new (C) LShiftINode(src_begin, intcon(1)));
...
bool LibraryCallKit::inline_dgemmDgemm() {
  ...
  Node* transa_start   = array_element_address(transa_value, transa_offset, T_BYTE);
  Node* transb_value   = load_String_value(NULL, transb);
  Node* transb_offset  = load_String_offset(NULL, transb);
  Node* transb_start   = array_element_address(transb_value, transb_offset, T_BYTE);
...
bool LibraryCallKit::inline_dgemvDgemv() {
  ...
  Node* trans_start   = array_element_address(trans_value, trans_offset, T_BYTE);
