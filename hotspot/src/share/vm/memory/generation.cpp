...
size_t Generation::max_capacity() const {
  ...
  if (Universe::is_dynamic_max_heap_enable()) {
    guarantee(_virtual_space.dynamic_max_heap_size() <= reserved().byte_size(), "must be");
    return _virtual_space.dynamic_max_heap_size();
  }
...
void CardGeneration::compute_new_size() {
  ...
  // Dynamic Max Heap
  const size_t exp_size = exp_dynamic_max_heap_size();
  ...
  if (capacity_after_gc < minimum_desired_capacity) {
    ...
    if (Universe::is_dynamic_max_heap_enable() && exp_size > committed_size()) {
      DMH_LOG("Gen_ElasticMaxHeapOp: CardGeneration::compute_new_size expand fails shrink:"
              "  minimum_desired_capacity: %6.1fK"
              "  real capacity: %6.1fK"
              "  exp_dynamic_max_heap_size: %6.1fK",
              minimum_desired_capacity / (double) K,
              capacity() / (double) K,
              exp_size / (double) K);
    }
  ...
  if (Universe::is_dynamic_max_heap_enable() &&
    ...
    DMH_LOG("Gen_ElasticMaxHeapOp: CardGeneration shrink according to ElasticMaxHeap");
  }
  ...
  if (Universe::is_dynamic_max_heap_enable() && exp_size > 0) {
    ...
    DMH_LOG("Gen_ElasticMaxHeapOp: CardGeneration ElasticMaxHeap adjust %s, expect "
...
OneContigSpaceCardGeneration::expand_and_allocate(size_t word_size,
  ...
  guarantee(!Universe::is_dynamic_max_heap_enable(), "OneContigSpaceCardGeneration do not support Dynamic Max Heap");
  if (parallel) {
