...
final class StringUTF16 {
    ...
    public static int indexOfLatin1(byte[] src, int srcCount, byte[] tgt, int tgtCount, int fromIndex) {
        ...
        checkBoundsBeginEnd(0, tgtCount, tgt.length);
     ...
     */
    static void checkIndex(int index, byte[] val) {
        if (index < 0 || index >= length(val)) {
     ...
     */
    static void checkOffset(int offset, byte[] val) {
        if (offset < 0 || offset > length(val)) {
    ...
    static void checkBoundsOffCount(int offset, int count, byte[] val) {
        if (offset < 0 || count < 0 || offset > length(val) - count) {
            throw new StringIndexOutOfBoundsException(
                    "offset " + offset + ", count " + count + ", length " + length(val));
        }
    }
    ...
    static void checkBoundsBeginEnd(int begin, int end, byte[] val) {
        if (begin < 0 || begin > end || end > length(val)) {
            throw new StringIndexOutOfBoundsException(
                    "begin " + begin + ", end " + end + ", length " + length(val));
        }
    }
