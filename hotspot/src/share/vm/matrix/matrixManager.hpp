...
#include "matrix/fileTable.hpp"
#include "matrix/matrixUtils.hpp"
#include "matrix/allowList.hpp"

class MappedFileInfo : public CHeapObj<mtInternal> {
public:
  int fake_fd;
  ...
  explicit MappedFileInfo(int fd) :
    fake_fd(fd), length(0), offset(0), name(NULL),
    head_blk(NULL), cur_blk(NULL), cur_blk_off(0), cur_blk_size(0) {}
};

class MappedFileInfoTable : public MatrixHashtable<int, MappedFileInfo, mtInternal> {
public:
  MappedFileInfoTable() : MatrixHashtable()
  {
    struct rlimit rlp;
    int status = getrlimit(RLIMIT_NOFILE, &rlp);
    if (rlp.rlim_max < 0 || rlp.rlim_max >= INT_MAX) {
      guarantee(false, "must be: more check handle");
    } else {
...
public:
  ...
  long transfer(int dst, int src, long offset, long count);
  int   is_ub_file(const char* path);
  bool  is_ub_addr(void* addr);
  int   fallback(int fake_fd);

private:
  MappedFileInfoTable _file_info_table;
};

class MatrixGlobal: AllStatic {
public:
