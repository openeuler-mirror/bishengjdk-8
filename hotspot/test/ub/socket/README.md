...
| `transferTo` | `transferTo <host> <port> <size> <id>` | Writes a plain temporary file to a UBSocket channel using `FileChannel.transferTo`, verify hash |
...
- `TransferToTest.java`: normal file `FileChannel.transferTo` to an attached UBSocket without enabling UBFile using 8 MiB, plus UBFile + UBSocket enabled transferTo using 16 MiB, including heartbeat progress and UBFile transfer log assertions.
