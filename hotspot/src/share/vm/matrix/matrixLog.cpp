...
static const char* ub_feature_name(UBFeature feature) {
  switch (feature) {
    case UB_FILE:   return "file";
    case UB_SOCKET: return "socket";
    case UB_HEAP:   return "heap";
    default:        return "unknown";
  }
}
...
void MatrixLog::apply_selector(const char* token_start, const char* token_end) {
  ...
  if (equals < token_end) {
    if (word_equals(feature_start, feature_len, "file_path")) {
      set_log_path(UB_FILE, equals + 1, token_end);
      return;
    }
    ...
    if (word_equals(feature_start, feature_len, "heap_path")) {
      set_log_path(UB_HEAP, equals + 1, token_end);
      return;
    }
  }
